...
const SECTION: TransportSection[] = [
  {
    ...
    stats: [
      { value: "35", label: "Locomotives", description: "Rail Efficiency" },
      {
        value: "805",
        label: "Wagons",
        description: "Flexibility for every transport",
      },
      { value: "3", label: "Licenses", description: "Global Scope" },
      {
        value: "2",
        label: "Terminals",
        description: "Strategically positioned",
      },
    ],
    chips: [
      "Cost optimization",
      ...
      {data.map((point) => (
        <div key={point.year} className="flex flex-1 flex-col items-center">
          <div className="flex h-[150px] w-full flex-col justify-end items-center">
            ...
            <div
              className="w-8 rounded-sm bg-blue-600"
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
        ...
        <div className="mt-6 flex flex-wrap gap-3 border-t border-gray-200 pt-5">
          {section.chips.map((chip) => (
            <span
              key={chip}
              className="inline-flex items-center gap-2 rounded-full border border-blue-500 px-4 py-1.5 text-sm text-blue-600 hover:bg-blue-100 transition ease-in-out duration-200"
            >
              <CheckIcon />
              {chip}
            </span>
